...
import MobileHeaderTablet from "../../Components/MobileHeaderTablet";
...
export default class MobileHomeTablet extends Component {
  ...
  getData(
    ...
    let uri = `${BE_Get_Product}/lite?outlet_id=${gerai_id}${is_favorite}&name=${search}&status=active&per_page=999&page=1`;
    ...
    NetInfo.fetch().then(state => {
      ...
      if (state.isConnected) {
        fetch(uri, {
          ...
          .then(response => response.json())
          .then(responseJson => {
            //console.log("GetFavMenu ==> ", responseJson);
  ...
  getMenuByCategory(
    ...
    let uri = `${BE_Get_Product}/lite?outlet_id=${gerai_id}&product_category_id=${activeCategory}&name=${search}&status=active&page=1`;
  ...
  addOrder() {
    ...
    let dataAdd = {
      ...
      price: productPrice,
      total: (salesTypeValue + productPrice) * selectedQuantity,
  ...
  checkOutUpdate(action = "") {
    ...
    let items = [];
    console.log("dataBill Update ==> ", detail);
    detail.map((v, i) => {
      let addons = [];
      v.detail.map((val, index) => {
        //const contoh_data = {id: 6, name: "Sosis", price: 2000, parentId: 3}
      ...
      <View>
       ...
       > */}
        <View
          style={{
            flex: 1,
            width: this.state.tablet ? (width - 120) / 2 : width,
            //width: "120%",
            //width: "100%",
            justifyContent: "space-between",
            flexDirection: "row",
      ...
      >
        ...
        <View
          style={{
            //flex: 1,
            justifyContent: "space-between",
            width: this.state.tablet ? "80%" : "100%",
            alignSelf: "center"
            // flexDirection: "row"
          }}
        >
            ...
            >
              ...
              >
                ...
                this.state.selectedSalesType.toString() ===
                  this.state.sales_type_dine_in_id.toString()
                  ? _simpan[this.state.languageIndex]
                  : _lanjut_short[this.state.languageIndex]}
              </Text>
            </Button>
          </View>
  ...
  reprintKitchen(update, retry = 0, type) {
    ...
    if (parseInt(retry) < 4) {
      this.connect(this.state.printer_kitchen.address);
      setTimeout(() => {
        this.printKitchen(update, retry, type);
      }, 750);
    }
  }
          ...
          >
            ...
            <View
              style={{
                ...
                marginTop: 0,
                paddingTop: 10,
                display: this.state.formItem ? "flex" : "none"
              }}
            >
              {this.renderFormItem_V2()}
            </View>
          </View>
