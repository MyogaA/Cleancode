...
import { _lanjut_short, _no_table } from "../../Libraries/DictionaryHome";
...
export default class MobileHistory extends Component {
  ...
  PrintInternal()
  {
    ...
    if (split_bill_data.length > 1) {
      ...
      jum_discount = selectedData.Payment.payment_discount;
      jum_tax = selectedData.Payment.payment_tax;
      jum_service = selectedData.Payment.payment_service;
    ...
    dataBill.map((data, i) => {
      ...
      //name
      nameData.push(`${product_name}${dt}${notes}`);
      //qty
    ...
    title = `***REPRINT***\n\r`;
    let text_table = table_id !== 0 ? `${_no_table[this.state.languageIndex]} ${no_table}\n\r` : "";
    let text_cashier = cashier_name ? `${_cashier[this.state.languageIndex]} ${cashier_name}\n\r` : "";
    head = `${gerai_name}\n${description}\n${text_table}\n${_transaction_id[this.state.languageIndex]} ${transaction_id}\n${text_cashier}\n${time}\n`;


    sub_total = sub_total;
    discount = jum_discount;
    tax = jum_tax;
    services = jum_service;
    total = grand_total;

    const bayar = total_bayar;
    const kembali = total_bayar - grand_total;

    AutoPrintFunctions.PrintNoInvoiceFunction(title, head, tail, invoice, sub_total, discount, tax, services, total, bayar, kembali, nameData, numData, amountData, v => {
      // alert("success print");
  ...
  changeSelectionbackupnew(data, refund_process = false) {
    const { userInfo } = this.state;
    ...
    let uri_get = `${BE_Transaction}/${data.id}`;
      ...
      >
        ...
        >
          ...
          <Text
            style={[
              MainStyle.robotoNormal,
              { fontSize: 15, color: BLACK, marginTop: 10, marginBottom: 10 }
            ]}
          >
      ...
      >
        ...
        >
          <Text
            style={[MainStyle.robotoNormalBold, { color: BLACK, fontSize: 14 }]}
          >
            {_by_status[this.state.languageIndex]} STATUSNYA
          </Text>
          <View
            style={{
              marginTop: 10,
        ...
        >
          ...
          >
            {_by_payment[this.state.languageIndex]} BAYAR
          </Text>
          <FlatList
              ...
              <View style={{ width: "100%", alignItems: "flex-start", flexDirection:"row" }}>
                ...
                <View style={{width: "20%"}}>
                  <Text
                    style={[
                      MainStyle.robotoNormalBold,
                      { fontSize: 13, color: status_color[status_index] }
                    ]}
                  >
                    {status_text}
                  </Text>
  ...
  renderHistoryLeftSide() {
    ...
    return (
      ...
      >
        <View style={{ margin: 15, flex: 1, marginRight:5, backgroundColor:WHITE, borderRadius: 15 }}>
          {this.renderSearch()}
  ...
  renderHeaderDetail(data) {
    ...
    return (
      <View style={{ marginTop: 0, display: has_items ? "flex" : "none" }}>
        <View
          style={{
            flexDirection: "row",
            justifyContent: "space-between",
  ...
  sendReceipt() {
    ...
    fetch(uri, {
      ...
      .then(response => response.json())
      .then(responseJson => {
        console.log("responseJson SENNNND EMAILLLL===> ", responseJson);
      ...
      <View style={[ss.body]}>
        ...
        >
          ...
          >
            ...
            <View style={{ alignItems: "center", flex: 46.25 }}>
              {this.renderHistoryLeftSide()}
            </View>

            <View style={{ alignItems: "center", flex: 46.25 }}>
              {refund ? (
                <View style={{ flex: 1, margin: 15, marginLeft:5,paddingLeft: 15, paddingRight: 15, borderRadius: 15, backgroundColor: WHITE }}>
