...
export default class MobileGobizTransaction extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  reprintBill(update, retry = 0) {
    console.log("retry kitchen ====> ", retry);

    if (parseInt(retry) < 6 && this.state.printer_main) {
      this.connect(this.state.printer_main.address);
      setTimeout(() => {
        this.printBill(null, retry);
      }, 750);
    }
  }
      ...
      dataBill.map((data, i) => {
        ...
        if (true) {
          ...
          BluetoothEscposPrinter.printColumn(
            [32 + total_addition_name, 1, 11 + total_addition_price],
            [
              BluetoothEscposPrinter.ALIGN.LEFT,
              BluetoothEscposPrinter.ALIGN.LEFT,
  ...
  async printBill(update, times = 1) {
    try {
      const { userInfo, dataBill, selectedData, printType } = this.state;
      let address = "Alamat dari Retail";
      let gerai_name = userInfo.gerai_name;
      ...
      dataBill.map((data, i) => {
        ...
        if (true) {
          ...
          BluetoothEscposPrinter.printColumn(
            [24 + total_addition_name, 6, 12 + total_addition_price],
            [
              BluetoothEscposPrinter.ALIGN.LEFT,
              BluetoothEscposPrinter.ALIGN.LEFT,
              BluetoothEscposPrinter.ALIGN.RIGHT
          ...
          if (notes_length > 0) {
            ...
            BluetoothEscposPrinter.printColumn(
              [32 + total_addition_name, 1, 1 + total_addition_price],
              [
                BluetoothEscposPrinter.ALIGN.LEFT,
                BluetoothEscposPrinter.ALIGN.LEFT,
                BluetoothEscposPrinter.ALIGN.RIGHT
      ...
      dataBill.map((v, i) => {
        let temp_total = v.price * v.quantity;
      ...
      await BluetoothEscposPrinter.printColumn(
        [26 + total_addition_name, 1, 15 + total_addition_price],
        [
          BluetoothEscposPrinter.ALIGN.LEFT,
          BluetoothEscposPrinter.ALIGN.LEFT,
          BluetoothEscposPrinter.ALIGN.RIGHT
      ...
      const tax_render = subTotal * this.state.tax_rate;

      const services_render = subTotal * this.state.service_rate;
      ...
      if (printType === 2) {
        if (tax_total_length_2 < 25 + 16) {
          tax_total_space_num_2 = 25 + 16 - tax_total_length_2;
        }
      }
      ...
      let grand_total = subTotal + services_render + tax_render;
      grand_total = grand_total.toString();

      let tax_total_length_3 = grand_total.length;
      let tax_total_space_3 = "";
      let tax_total_space_num_3 = 0;
      if (tax_total_length_3 < 25) {
        tax_total_space_num_3 = 25 - tax_total_length_3;
      }

      if (printType === 2) {
        if (tax_total_length_3 < 25 + 16) {
          tax_total_space_num_3 = 25 + 16 - tax_total_length_3;
        }
      }

      for (var xxxx = 0; xxxx < tax_total_space_num_2; xxxx++) {
        tax_total_space_3 = tax_total_space_3 + " ";
      }
      ...
      await BluetoothEscposPrinter.printColumn(
        ...
        [`${_grand_total[this.state.languageIndex]}`, " ", `${tax_service}`],
        {
          encoding: "GBK",
          codepage: 0,
          widthtimes: 0,
      ...
      if (times < 3) {
        //this.printKitchen(update, times + 1);
        //if (type === "kitchen") {
        this.printBill(update, parseInt(times) + 1);
        //} else {
        //this.reprintKitchenMain(update, parseInt(times) + 1, type);
        //}
      }
  ...
  getBEData(page = this.state.page, status_category = "active") {
    const {
      userInfo,
      searchKey,
      startDate,
    ...
    // }
    if (status_category) {
      status = "&status_category=" + status_category;
    }

    // let uri = `${BE_Transaction}?order=newest&outlet_id=${outlet_id}&business_id=${business_id}${status}`;
    let uri = `${BE_Gobiz_Transaction}?order=newest&outlet_id=${outlet_id}${status}&page=${page}&per_page=100`;
      ...
      if (state.isConnected) {
        fetch(uri, {
          ...
          .then(responseJson => {
            ...
            console.log("result_all_gobiz ===> ", result);

            if (resultData.statusCode === 200) {
              this.setState({
                listHistory: result_all,
                loading: false
              });
            }
  ...
  changeSelection(data, refund_process = false) {
    ...
    data.Gobiz_Transaction_Items.map((v, i) => {
      //console.log('v ==> ', v);
      dataBill.push(v);
    });
  ...
  renderList(data, i) {
    ...
    const status_color = ["#000000", "#BA1818", "#37A441", "#C4C4C4"];

    let status_text = "";
    // status_text = status_text.charAt(0).toUpperCase() + status_text.slice(1);
    // status_text === "new" ? status_text === "New" : "";
    // status_text === "onprocess" ? status_text === "On Process" : "";
    // status_text === "finding_driver" ? status_text === "Find Driver" : "";
    // status_text === "system_cancel" ? status_text === "System Cancel" : "";
    // status_text === "merchant_cancel" ? status_text === "Merchant Cancel" : "";
    // status_text === "user_cancel" ? status_text === "User Cancel" : "";
    // status_text === "on_delivery" ? status_text === "On Delivery" : "";
    // status_text === "success" ? status_text === "Success" : "";
    // status_text === "driver_arrived" ? status_text === "Driver Arrived" : "";

    if (status === "new") {
      status_text = "New";
    }
    if (status === "onprocess") {
      status_text = "On Process";
    }
    if (status === "finding_driver") {
      status_text = "Find Driver";
    }
    if (status === "system_cancel") {
      status_text = "System Cancel";
    }
    if (status === "merchant_cancel") {
      status_text = "Merchant Cancel";
    }
    if (status === "user_cancel") {
      status_text = "User Cancel";
    }
    if (status === "on_delivery") {
    ...
    if (data.Gobiz_Driver) {
      driver_name = data.Gobiz_Driver.driver_name;
    }
    if (show) {
      return (
        <View
                ...
                <Text
                  style={[
                    MainStyle.robotoNormalBold,
                    {
                      fontSize: 11,
                      color: textColor[colorIndex],
                      display: driver_name ? "flex" : "none"
                    }
                  ]}
                >
                  {_driver[this.state.languageIndex]}
                </Text>
                <Text
                  style={[
                    MainStyle.robotoNormal,
  ...
  renderHeaderDetail(data) {
    ...
    let status = data.status ? data.status.toString() : "";
    const status_color = [
      "#000000",
      "#BA1818",
      "#BA1818",
  ...
  renderButtonCommand() {
    ...
    if (status === "new") {
      status_category = "new";
      button_text = _proses_transaksi_gobiz_1[this.state.languageIndex];
    }
    ...
    if (
      ...
      status_category = "process";
      button_text = _proses_transaksi[this.state.languageIndex];
    }
  ...
  prosesTransaksi() {
    ...
    if (status_category === "new") {
      this.updateStatusTransaksi("finding_driver");
      //this.updateStatusTransaksi("driver_arrived");
      // this.printKitchen();
      this.connect(this.state.printer_kitchen.address);
    ...
    if (status_category === "process") {
      this.updateStatusTransaksi("on_delivery");
      // this.updateStatusTransaksi("success");
      // this.printKitchen();
      this.connect(this.state.printer_main.address);
  ...
  batalkanTransaksi() {
    ...
    let uri = `${BE_Gobiz_Transaction}/update-status/${transaction_id}`;
    ...
    fetch(uri, {
      ...
      body: JSON.stringify({
        status: "merchant_cancel",
        reason_cancel: this.state.notes
      })
    })
      .then(response => response.json())
      .then(responseJson => {
  ...
  updateStatusTransaksi(status) {
    ...
    fetch(uri, {
      ...
      .then(response => response.json())
      .then(responseJson => {
        console.log("Response Update Status Transaksi ==> ", responseJson);
        let result = responseJson;
        ...
        >
          ...
          <Button
            onPress={() => {
              ...
              this.getBEData(this.state.page, "all");
            }}
            style={{
              //margin: 15,
              marginTop: 0,
          ...
          <Button
            onPress={() => {
              ...
              this.getBEData(this.state.page, "active");
            }}
            style={{
              //margin: 15,
              marginTop: 0,
          ...
          <Button
            onPress={() => {
              ...
              this.getBEData(this.state.page, "success");
            }}
            style={{
              //margin: 15,
              marginTop: 0,
          ...
          <Button
            onPress={() => {
              ...
              this.getBEData(this.state.page, "cancel");
            }}
            style={{
              //margin: 15,
              marginTop: 0,
