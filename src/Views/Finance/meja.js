...
export default class Meja extends Component {
    ...
    if (action === 1) {
      ...
      tempData.map((item, index) => {
        ...
        if (item.status === 2 && data.status !== 1) {
          ...
          valid = false;
          reasonNotValid.push("Kapasitas meja tidak cukup.");
        }
      ...
      >
        ...
        >
          <Text
            style={[
              MainStyle.dmSansBold,
              { color: BLACK, fontSize: 40, marginBottom: 15 }
            ]}
          >
            {data.name}
          </Text>
  ...
  renderTableMove() {
    ...
    return (
      ...
      >
        <View style={[ss.modalCover]}>
          <View style={[ss.modalBox, { width: "50%" }]}>
            <View style={{ position: "absolute", right: 10, top: 10 }}>
              <Button
                onPress={() => {
                  this.closeModalMove();
