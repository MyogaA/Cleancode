...
export default class MobileRekap extends Component {
      ...
      >
        ...
        >
          ...
          <Text
            style={[
              MainStyle.robotoNormal,
              { fontsize: 14, color: BLACK, marginTop: 10, marginBottom: 10 }
            ]}
          >
  ...
  PrintRekapInternal()
  {
    ...
    nameRekapData.push(`${_penjualan[this.state.languageIndex]}`);
    amountRekapData.push(parseInt(sales));

    nameRekapData.push(`${_void[this.state.languageIndex]}`);
    amountRekapData.push(parseInt(jum_void));

    nameRekapData.push(`${_cash_in_out[this.state.languageIndex]}`);
    amountRekapData.push(parseInt(cash_in));

    nameRekapData.push(`${_tunai[this.state.languageIndex]}`);
    amountRekapData.push(parseInt(cash));
    ...
    if (recap_items_card && recap_items_card.length > 0) {
      recap_items_card.map((v, i) => {
        ...
        if (v.total > 0) {
          nameRekapData.push(`${v.name}`);
          amountRekapData.push(parseInt(total_value));
        }
      });
    }
    ...
    let aktual = total_actual - total_mdr;

    nameRekapData.push(`${_jumlah_aktual[this.state.languageIndex]}`);
    amountRekapData.push(parseInt(aktual));

    nameRekapData.push(`${_selisih_1[this.state.languageIndex]}`);
    amountRekapData.push(parseInt(difference));


    let mdr_text = "";

    if (calculateMdr) {
      mdr_text = `${_with_mdr[this.state.languageIndex]}\n\r`
    }



    let title = `${restaurant_name}`;
    let head = `${restaurant_address}\n\r${time}\n\r${_kasir[this.state.languageIndex]} ${namaKasir}\n\r${mdr_text}`;
    let tail = "\r\r\r";
    let invoice = "\n";
    ...
    this.state.recap_items.map((data, index) => {
      ...
      let notes = data.notes ? `\r${data.notes}` : "";
      //name
      nameData.push(`${data.name}${notes}`);
      //qty
      numData.push(parseInt(data.quantity));
      //total
      amountData.push(parseInt(data.price_total));
    ...
    AutoPrintFunctions.PrintRecapFunction(title, head, tail, invoice, nameData, numData, amountData, nameRekapData, amountRekapData, v => {
      // alert("success print");
      this.setState({printingNow: false});
    })
    ...
    try {
      ...
      if (this.state.recap_items.length > 0) {
        ...
        this.state.recap_items.map((data, index) => {
          ...
          BluetoothEscposPrinter.printColumn(
            [18 + total_addition_name, 4, 10 + total_addition_price],
            [
              BluetoothEscposPrinter.ALIGN.LEFT,
              BluetoothEscposPrinter.ALIGN.LEFT,
              BluetoothEscposPrinter.ALIGN.RIGHT
  ...
  getRekapListFirstTime() {
    ...
    fetch(uri, {
      ...
      .then(responseJson => {
        ...
        if (result.statusCode === 200) {
          let resultData = result.data;
          console.log("getData first time ==> ", result);
          ...
          let new_date_start = moment(new Date())
            .add(-60, "days")
            .format("2000-MM-DD 00:00");
      ...
      .then(responseJson => {
        ...
        if (responseJson.statusCode === 200) {
          ...
          payment_method_card.map((v, i) => {
            ...
            //console.log("mdr card ===> ", mdr_total)
  ...
  //         >
  //           <Text
  //             style={[MainStyle.robotoNormalBold, { fontsize: 14, color: BLACK }]}
  //           >
  //             {startDate}
  //           </Text>
  //         </Button>
      ...
      >
        ...
        >
          <Text
            style={[
              MainStyle.robotoNormal,
              { fontsize: 12, color: textColor[colorIndex] }
            ]}
          >
            {date}
          </Text>
        ...
        >
          <View style={{ width: "33%" }}>
            <Text
              style={[
                MainStyle.robotoNormalBold,
                { fontsize: 13, color: textColor[colorIndex] }
              ]}
            >
      ...
      >
        ...
        >
          <Text
            style={[
              MainStyle.robotoNormalBold,
              { fontsize: 14, color: WHITE, margin: 5 }
            ]}
          >
            {_rekap[this.state.languageIndex]}
          </Text>
  ...
  printStruk(mdr = false) {
    this.setState({ calculateMdr: mdr });
      ...
      DeviceInfo.getDeviceName().then((deviceName) => {
        ...
        if (deviceName) {
          ...
          if (deviceName === "rk3566_r") {
            if (this.state.printingNow === false)
            {
              this.setState({printingNow: true});
              this.PrintRekapInternal();
            }
          }
          else
          {
        ...
        >
          ...
          <View style={{ width: "40%" }}>
            <Text
              style={[
                MainStyle.robotoNormal,
                { fontsize: 14, color: BLACK, marginTop: 0 }
              ]}
            >
              {data.notes}
            </Text>
    ...
    return (
      ...
      >
        ...
        <MaterialCommunityIcons
          ...
          style={{ fontsize: 120, color: "#9D9797" }}
        />
      </View>
    );
  }
  ...
  submitRekap() {
    ...
    console.log("parameters tablet ==> ", parameters);
    ...
    return (
      <View style={[ss.body]}>
        ...
        >
          ...
          <View style={{ flex: 46.5, backgroundColor: WHITE, margin: 10, marginRight:5 }}>
            {this.renderHistoryLeftSide()}
            {this.state.dataRekap.length === 0 ? this.renderNoData() : <View />}
          </View>

          <View style={{ flex: 46.5, backgroundColor: WHITE, margin: 10, marginLeft:5 }}>
