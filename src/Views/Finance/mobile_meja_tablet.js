...
export default class MobileMejaTablet extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  selectTable(data) {
    ...
    console.log("selectTable Current Action ===", this.state.action);
    if (action === 1) {
      //combine order/move meja
      // gabung/pisah
      // console.log("tempData action 1 ===> " ,tempData);
      // console.log("data action 1 ===> " ,data);

      if (data.status === "available" && tempData.length !== 1) {
        //select first data wrong
        console.log("first data is not valid");
        valid = false;
      }
      if (data.status !== "available" && tempData.length === 0) {
        //select first data
        console.log("first data is selected");
        let dummy = this.state.selectedTable;
        dummy.push(data);
        tempData.push(data);
        this.setState({ selectedTable: dummy });
        valid = true;

        console.log(
          "tempData.length after select first data ===",
          tempData.length
        );
      }

      console.log("tempData.length general ===", tempData.length);
      ...
      if (tempData.length > 1) {
        tempData.map((item, index) => {
          console.log("tempData Map action item 1 ==> ", item.id);
          console.log("tempData Map action data 1 ==> ", data.id);
      ...
      // console.log("data MEJA ==> ", data);//select table biasa
      tempData.map((item, index) => {
        if (item.id === data.id) {
          tempData.splice(index, 1);
          isSame = true;
  ...
  getTableList(search = this.state.searchKey, activeFloor = this.state.activeFloor) {
    const { userInfo } = this.state;
    ...
    let floor_search = activeFloor !== 0 ? `&floor_id=${activeFloor}` : "";

    let uri = `${BE_TableManagement}?outlet_id=${outlet_id}${floor_search}${name_search}&page=1&per_page=100`;
    ...
    fetch(uri, {
      ...
      .then((responseJson) => {
        ...
        {
          this.setState({loading: false, listTable:[],selectedTable: []});
  ...
  renderRightSide() {
    ...
    return (
      ...
      >
        ...
        <Button
        onPress={() => {
          this.setState({activeFloor: 0})
          this.getTableList(this.state.searchKey, 0);
        }}
        style={{
          marginTop: 0,
          alignContent: "center",
          ...
          flexDirection: "row",
          padding: 10,
          backgroundColor: this.state.activeFloor === 0 ? 
  ...
  renderfloorData(data, index)
  {
    return(
      <Button
        onPress={() => {
          this.setState({activeFloor: data.id});
          this.getTableList(this.state.searchKey, data.id);
        }}
        style={{
          marginTop: 0,
          //width: "100%",
          ...
          flexDirection: "row",
          padding: 10,
          backgroundColor: this.state.activeFloor === data.id ? 
            MAIN_THEME_COLOR_SELECT(this.state.colorIndex) : WHITE,
          marginRight: 5,
          borderRadius: 5
          //alignItems: 'center',
        ...
        <Text
          style={[
            MainStyle.robotoNormalBold,
            {
              fontSize: 12,
              color:
                this.state.activeFloor === data.id
                  ? WHITE
                  : MAIN_THEME_COLOR_SELECT(this.state.colorIndex)
            ...
            >
              ...
              {status === "used" ? (
                [
                  ...
                  <Button
                    style={[ss.modalContent, {display: "none"}]}
                    onPress={() => {
