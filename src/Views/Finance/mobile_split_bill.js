...
export default class MobileSplitBill extends Component {
  constructor(props) {
    super(props);
    this.state = {
      logo_image_base64: "",
      ...
      if (state.isConnected) {
        fetch(uri, {
          ...
          .then(responseJson => {
            ...
            let result = responseJson;
            let resultData = result.data;
            //console.log("BE_Payment_Method ==> ", resultData);
  ...
  setInformation() {
    ...
    console.log("set information bill_services ===> ", bill_services);
    console.log("set information bill_tax ===> ", bill_tax);

    console.log("set information bill_discount ===> ", bill_discount);
    console.log("set information grand_total ===> ", grand_total);
  ...
  async printKitchenBahasa(reconnect) {
    try {
      const {
        userInfo,
        dataBill,
...
Pembayaran : ${this.idrNumToStr(grand_total)}
...
Grand total : ${this.idrNumToStr(grand_total)}
Payment : ${this.idrNumToStr(grand_total)}
...
Powered By BeetPOS`;
  ...
  async printActionEng(reconnect, kitchen) {
    // BluetoothManager.connect(printer.address) // the device address scanned.
    //   .then(
    //     s => {
    //       this.setState({
    ...
    try {
      ...
      //if (this.state.show_order_id) {
      await BluetoothEscposPrinter.printText(
        `Transaction ID ${transaction_id}-SPT\n\r`,
        {
          encoding: "GBK",
          codepage: 0,
          widthtimes: 0,
      ...
      dataPisahNominal.map((v, i) => {
        let total_length = v.jumlah.toString().length;
        let total_space = "";
        let total_space_num = 0;

        let payment_type = this.state.payment_type;

        this.state.payment_method_all.map((item, value) => {
          console.log("splitbayar cetak struk v ===> ", item);
          const id = parseInt(item.id);
          if (id === parseInt(v.payment)) {
        ...
        BluetoothEscposPrinter.printText(
          `${payment_type}${total_space}${v.jumlah}\n\r`,
          {}
        ); ////total bayar disini
      });
  ...
  async printActionBahasa(reconnect, kitchen) {
    // BluetoothManager.connect(printer.address) // the device address scanned.
    //   .then(
    //     s => {
    //       this.setState({
    ...
    try {
      ...
      //if (this.state.show_order_id) {
      await BluetoothEscposPrinter.printText(
        `ID Transaksi ${transaction_id}-SPT\n\r`,
        {
          encoding: "GBK",
          codepage: 0,
          widthtimes: 0,
  ...
  checkOutAction(times = 0) {
    ...
    NetInfo.fetch().then(state => {
      ...
      if (state.isConnected) {
        ...
        dataPisahNominal.map((v, i) => {
          const jumlah = this.state.currencyAllowDecimal
            ? parseFloat(v.jumlah).toFixed(2)
            : parseInt(v.jumlah);
          const total_tax = tax_render;
          const total_services = services_render;
          const ratio = parseFloat(jumlah / grandTotal);

          const jumlah_tax = this.state.currencyAllowDecimal
            ? parseFloat(total_tax * ratio).toFixed(2)
            : parseInt(total_tax * ratio);

          const jumlah_services = this.state.currencyAllowDecimal
            ? parseFloat(total_services * ratio).toFixed(2)
            : parseInt(total_services * ratio);

          const jumlah_payment = this.state.currencyAllowDecimal
            ? parseFloat(v.jumlah).toFixed(2)
            : parseInt(v.jumlah);

          let temp_data = {
            payment_method_id: parseInt(v.payment),
            payment_discount: 0,
            payment_tax: jumlah_tax,
            payment_service: jumlah_services,
            payment_total: jumlah_payment,
            amount: jumlah_payment,
            payment_change: 0
          };

          payment.push(temp_data);
        });

        let parameter_send = {
          receipt_id: data_post_1.kode + "-SPT",
        ...
        console.log("payment split parameters ==> ", payment);
        ...
        if (order_id) {
          ...
          fetch(uri, {
            ...
            .then(responseJson => {
              ...
              if (responseJson.statusCode === 201) {
                MenuFunctions.ClearNewMenuAll(val => {}); //comment
                ...
                this.setState({
                  //transaction_id: transaction_id,
                  transaction_id: responseJson.data[0].id,
                  transaction_success: responseJson.data,
                  showPaymentSuccess: true,
                  loading: false,
                  dine_in_print: true
                });
          ...
          let uri = `${BE_Create_Transaction}split`;
          console.log("uri ==> BE_Create_Transaction ", BE_Create_Transaction);
          ...
          fetch(uri, {
            ...
            .then(responseJson => {
              ...
              if (responseJson.statusCode === 201) {
                ...
                console.log("Loading False ===> Show Payment Success True");
      ...
      >
        ...
        >
          <Text
            style={[MainStyle.robotoNormal, { color: BLACK, fontSize: 14 }]}
          >
            {_split_bill_1[this.state.languageIndex]}
          </Text>
        </View>
        <ScrollView
          showsVerticalScrollIndicator={false}
  ...
  splitBillBayar(index) {
    ...
    console.log("splitBillBayar tempData ===> ", tempData);

    this.state.payment_method_all.map((v, i) => {
      console.log("splitBillBayar v ===> ", v);
      const id = parseInt(v.id);
      if (id === parseInt(tempData.payment)) {
        if (v.qr_image) {
          this.setState({
            qr_image_url: BE_URI + v.qr_image,
            showQR: true,
            QRSelectedPayment: parseInt(tempData.jumlah),
            selectedIndex: index
          });
        } else {
          this.splitBillBayarProcess(index);
        }
      }
    });
  }

  splitBillBayarProcess(index) {
    let { dataPisahNominal } = this.state;
    let tempData = dataPisahNominal[index];
    tempData.status = 1;
  ...
  changeCaraBayar(index, value) {
    let { dataPisahNominal } = this.state;
    let tempData = dataPisahNominal[index];
    const jumlah = tempData.jumlah ? tempData.jumlah : 0;
    tempData.payment = value;
    dataPisahNominal[index] = tempData;

    // this.state.payment_method_all.map((v, i) => {
    //   //console.log("changeCaraBayar v ===> ", v.id);
    //   const id = parseInt(v.id);

    //   if (id === parseInt(value)) {
    //     console.log("changeCaraBayar v ===> ", v);

    //     if (v.qr_image) {
    //       this.setState({ qr_image_url: BE_URI + v.qr_image, showQR: true, QRSelectedPayment: jumlah});
    //     }
    //   }
    // });
                    ...
                    >
                      ...
                      >
                        <Dropdown
                          ...
                          text={_choose_payment[this.state.languageIndex]}
  ...
  render() {
    ...
    return (
      <View style={[ss.body]}>
        ...
        {showQR ? (
          <MobileShowQR
            ...
            actions={() => {
              ...
              this.splitBillBayarProcess(this.state.selectedIndex);
              //this.checkOutAction();
            }}
            closeActions={() => {
              this.setState({ showQR: false });
            ...
            grandTotal={this.idrNumToStr(this.state.QRSelectedPayment, true)}
          />
        ) : (
          <View />
        )}
        ...
        <MobileHeader
          colorIndex={this.state.colorIndex}
          title={_split_bill[this.state.languageIndex]}
          notif={false}
          hideLogin={true}
        ...
        >
          <View style={{}}>
            ...
            >
              ...
              >
                {_pembayaran_selesai[this.state.languageIndex]}asd
              </Text>
            </Button>
          </View>
        </View>
