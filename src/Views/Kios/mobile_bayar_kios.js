...
export default class MobileBayar extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      customerNumber: this.props.customerNumber ? this.props.customerNumber: null,
      ppob: this.props.ppob ? this.props.ppob : false,
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        { 
          ...
          console.log("this.state.selectedProduct.price ===>" , this.state.selectedProduct.price);
          console.log("this.state.customerNumber ===>" , this.state.customerNumber);
        
          // this.setState({
          //   bill_subTotal: this.state.selectedProduct.price,
          //   grandTotalRound : this.state.selectedProduct.price
          // });
        }
  ...
  setInformation() {
    ...
    if (this.state.ppob)
    {
      subTotal = this.state.selectedProduct.price;
      grandTotalRound = this.state.selectedProduct.price;
    }
            ...
            if (order_id) {
              ...
              if (this.state.direct_payment) {
                ...
                if (this.state.ppob)
                {
                  ...
                  let uri_kios_transaction = BE_Kios_Transaction + "/payment-transaction";
                  let body_kios_transaction = 
                  {
                    business_id : business_id,
                    outlet_id: outlet_id,
                    kios_product_id: this.state.selectedProduct.id,
                    receipt_id: this.state.bill_transId,
                    number: this.state.customerNumber,
                    customer_account_id: 1,
                    customer_id:1,
                    description: "PPOB TEST",
                    payment_discount: 0,
                    payment_tax: 0,
                    payment_service: 0,
                    payment_total: this.state.selectedProduct.price,
                    amount: this.state.cashAmount,
                    payment_fee: 0,
                    payment_admin: 0,
                    payment_change: this.state.cashAmount - this.state.selectedProduct.price ,
                    status: "done"
                  }
                  
                  fetch(uri_kios_transaction, {
                    method: "POST", //CREATE KIOS TRANSACTION
                    headers: {
                      Accept: "application/json",
                      "Content-Type": "application/json",
                      Authorization: this.state.auth
                    },////
                    body: JSON.stringify(body_kios_transaction)
                  })
                  .then(response => response.json())
                  .then(responseJson => {
                    console.log("Response Kios uri ==> ", uri_kios_transaction);
                    console.log("Response body_kios_transaction ==> ", body_kios_transaction);



                    console.log(
                      "Response Payment BE_Kios_Transaction ==> ",
                      responseJson
                    );
