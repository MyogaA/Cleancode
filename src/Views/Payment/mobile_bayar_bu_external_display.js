...
export function DualMonitor(state) {
  ...
  if (payment_type === "cash" || payment_type === "Cash") {
    ...
    payment_amount = parameters.grandTotal;
  }

  const payment_change = parameters.grandTotalRound - payment_amount;
...
export default class MobileBayar extends Component {
  ...
  async LoginCashlez() {
    CashlezFunctions.LoginCashlez(this.state.cz_user, this.state.cz_pin, v => {
  ...
  testBayar() {
    ...
    let body = {
      data: {
        request: {
          vendorIdentifier: this.state.cz_vendor_identifier,
          token: "",
          referenceId: `TEST-BEETPOS-${transaction_id}`,
          entityId: this.state.cz_entity_id,
          ...
          transactionUsername: this.state.cz_user
        }
      }
    };
  ...
  changeDiscount() {
    ...
    this.setState({
      grandTotal: grand_total > 0 ? grand_total : 0,
      //grandTotalRound: grandTotalRound < 0 ? 0 : grandTotalRound,// disable pembulatan
      grandTotalRound: grand_total > 0 ? grand_total : 0, // disable pembulatan
  ...
  *Thank you for shopping in ${business_name}-${gerai_name}*
  ...
  async printActionStarter(
    address = this.state.printer_main.address,
    retry = 0
  ) {
    // try {
    //   await BluetoothEscposPrinter.printText("\n\r", {});
    // } catch (error) {
    //   console.log("print starter error ===> ", error);
    //   this.connect(address);
    //   setTimeout(() => {
    //     this.connect(address);
    //     setTimeout(() => {
    //       this.connect(address);
    //       this.printActionStarterReTry();
    //     }, 300);
    //   }, 300);
    // }
  }

  async printActionStarterReTry(address = this.state.printer_main.address) {
    try {
      await BluetoothEscposPrinter.printText("\n\r", {});
    } catch (error) {
      console.log("print starter error retry ===> ", error);
    }
  }

  printActionLanguage(reconnect, kitchen, lunas, retry = 0) {
  ...
  async printAction(reconnect, kitchen, lunas, retry) {
    try {
      ...
      dataBill.map((v, i) => {
        ...
        // subTotal = subTotal + v.qty * v.salesTypeValue;
        subTotal = this.state.currencyAllowDecimal
          ? parseFloat(subTotal + temp_total).toFixed(2)
      ...
      if (this.state.currencyAllowDecimal) {
        discount = parseFloat(discount_render).toFixed(2);
      } else {
        discount = discount_render;
      }
      ...
      let tax_service = this.state.currencyAllowDecimal
        ? parseFloat(services_render)
            .toFixed(2)
            .toString()
        : services_render.toString();
  ...
  async printActionOld(reconnect, kitchen, lunas, retry) {
    // BluetoothManager.connect(printer.address) // the device address scanned.
    //   .then(
    //     s => {
    //       this.setState({
  ...
  async printActionBahasa(reconnect, kitchen, lunas, retry) {
    // BluetoothManager.connect(printer.address) // the device address scanned.
    //   .then(
    //     s => {
    //       this.setState({
    ...
    try {
      console.log("PRINT ACTION BAHASA");
      const {
        userInfo,
        dataBill,
        bill_transId,
      ...
      //await BluetoothEscposPrinter.printQRCode("TEST 123456789", 576, 1);
      ...
      console.log(
        "this.state.printer_kitchen.address ? ",
        this.state.printer_kitchen.address
      );
                    ...
                    if (this.state.access_print_bill) {
                      ...
                      setTimeout(() => {
                        ...
                        setTimeout(() => {
                          this.connect(this.state.printer_main.address);
                          this.printActionStarter(
                            this.state.printer_main.address
                          );
                          setTimeout(() => {
  ...
  cetakUlangDapur(retry = 0) {
    if (parseInt(retry) < 9) {
      this.connect(this.state.printer_kitchen.address);
      setTimeout(() => {
        this.printActionStarter(this.state.printer_kitchen.address);
        setTimeout(() => {
          this.connect(this.state.printer_kitchen.address);
          setTimeout(() => {
            console.log("REPRINT kitchen ===> ", retry);

            // if (parseInt(this.state.languageIndex) === 0) {
            //   this.printKitchen(false, parseInt(retry) + 1);
            // } else {
            //   this.printKitchenBahasa(false, parseInt(retry) + 1);
            // }
  ...
  cetakUlangKasir(retry = 0, kitchen = false) {
    if (parseInt(retry) < 9) {
      this.connect(this.state.printer_main.address);
      setTimeout(() => {
        this.printActionStarter(this.state.printer_main.address);
        setTimeout(() => {
          this.connect(this.state.printer_main.address);
          setTimeout(() => {
