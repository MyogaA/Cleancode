...
import { REGISTER_URI, FORGOT_PASSWORD_URI, BE_Business, BE_Business_No_Verify } from "../../Constants";
import {
  _ok_alert,
  _staff_id,
  _tidak_aktif
...
export default class MobileLogin extends Component {
  ...
  loginHandler(email = this.state.email, staff_id = this.state.staffId) {
    // let information = {
    //   name: "Dicky Yosep",
  ...
  loginversiontwo() {
    const full_staff_id = this.state.staffId;

    let words = full_staff_id.split("-");
    console.log(words)

    if (words.length > 1)
    {
    let business_id = parseInt(words[0]);
    console.log("business_id", business_id);

    const uri = BE_Business_No_Verify + business_id;
    ...
    fetch(uri, {
      ...
      .then(responseJson => {
        ...
        const email = responseJson.data.Owner.email;
        if (responseJson.statusCode === 200) {
          this.loginHandler(email, words[1]);
        }
        else
        {
    ...
    {
      alert("Format Staff Id is wrong.");
    }
  }
