...
import { _claim_method, _discount } from "../../Libraries/DictionaryPayment";
...
export default class MobileManagement extends Component {
    ...
    fetch(uri, {
      ...
      .then(responseJson => {
        ...
        if (result.statusCode === 200) {
          ...
          this.getCustomerVoucher(customer_id);
          this.getCustomerVoucherHistory(customer_id);
          this.getCustomerVoucherRedeem(customer_id);
        } else {
          this.setState({
            //refreshing: false,
            loading: false
  ...
  getCustomerVoucherHistory(customer_id, page = 1) {
    const { userInfo, selectedCustomer } = this.state;
    ...
    //status = "&status=refund";
    let per_page = 20;
    let uri = `${BE_Customer_Voucher_History}?order=newest&customer_id=${customer_id}&outlet_id=${outlet_id}&business_id=${business_id}&page=${page}&per_page=${per_page}${status}`;

    //let uri = `${BE_Transaction}?order=newest&outlet_id=${outlet_id}&business_id=${business_id}&page=1&per_page=5&date_start=${date_start}&date_end=${date_end}${status}`;

    console.log("getCustomerVoucherHistory BE ==> ", uri);
      ...
      if (state.isConnected) {
        fetch(uri, {
          ...
          .then(responseJson => {
            ...
            const statusCode = responseJson.statusCode;
            console.log(
              "result getCustomerVoucherHistory result ===> ",
              result
            );
            if (statusCode === 200) {
              const resultData = result;
              ...
              const pagination = resultData.pagination;
              let result_filtered = [];
              console.log("result getCustomerVoucherHistory ===> ", result_all);
              result_filtered = result_all;
              // if (filterPayment === 0) {
              //   result_filtered = result_all;
              // } else {
              ...
              // });
              this.setState({
                customer_voucher_history: result_all
  ...
  getCustomerVoucherRedeem(customer_id, page = 1) {
    const { userInfo, selectedCustomer } = this.state;
    ...
    //status = "&status=refund";
    let per_page = 20;
    let uri = `${BE_Customer_Voucher_Redeem}?order=newest&customer_id=${customer_id}&outlet_id=${outlet_id}&business_id=${business_id}&page=${page}&per_page=${per_page}${status}`;

    //let uri = `${BE_Transaction}?order=newest&outlet_id=${outlet_id}&business_id=${business_id}&page=1&per_page=5&date_start=${date_start}&date_end=${date_end}${status}`;

    console.log("getCustomerVoucherRedeem BE ==> ", uri);
      ...
      if (state.isConnected) {
        fetch(uri, {
          ...
          .then(responseJson => {
            const result = responseJson;
            const statusCode = responseJson.statusCode;
            // console.log("result getCustomerVoucherRedeem result ===> ", result);
            if (statusCode === 200) {
              const resultData = result;
              ...
              const pagination = resultData.pagination;
              let result_filtered = [];
              console.log("result getCustomerVoucherRedeem ===> ", result_all);
              result_filtered = result_all;
              // if (filterPayment === 0) {
              //   result_filtered = result_all;
              // } else {
              ...
              // });
              this.setState({
                customer_voucher_redeem: result_all
  ...
  getCustomerVoucher(customer_id, page = 1) {
    ...
    //status = "&status=refund";
    let per_page = 20;
    let uri = `${BE_Customer_Voucher_Personal}?order=newest&customer_id=${customer_id}&outlet_id=${outlet_id}&business_id=${business_id}&page=${page}&per_page=${per_page}${status}`;
  ...
  updateDataUser() {
    ...
    fetch(uri, {
      ...
      body: JSON.stringify({
        ...
        name: this.state.formName ? this.state.formName : "",
        notes: this.state.formNotes ? this.state.formNotes : "",
        address: this.state.formAddress ? this.state.formAddress : "",
        phone_number: this.state.formPhone ? this.state.formPhone : "",
        points: this.state.selectedCustomer.points,
        email: this.state.formEmail ? this.state.formEmail : "",
        bank: this.state.formBank ? this.state.formBank : "",
        no_rekening: this.state.formBankAccount ? this.state.formBankAccount : ""
        //image: image,
      })
    })
      .then(response => response.json())
  ...
  renderViewCustomer() {
    ...
    return (
      ...
      >
        ...
        {this.renderModalVoucherHistory()}
        {this.renderModalVoucherRedeem()}
            ...
            >
              ...
              {selectedCustomer.bank ? (
                <Text
                  style={[
                    MainStyle.robotoNormalBold,
                    { fontSize: 14, color: MAIN_THEME_COLOR_SELECT(this.state.colorIndex), marginTop: 2 }
                  ]}
                >
              ...
              >
                <Button
                  style={{
                    ...
                    alignItems: "center",
                    display:
                      this.state.transactions.length === 0 ? "none" : "flex"
                  }}
                  onPress={() => {
                    LayoutAnimation.configureNext(
                      LayoutAnimation.Presets.easeInEaseOut
                ...
                <Button
                  style={{
                    ...
                    alignItems: "center",
                    display:
                      this.state.transactions_point.length === 0
                        ? "none"
                        : "flex"
                  }}
                  onPress={() => {
              ...
              >
                <Button
                  style={{
                    ...
                    alignItems: "center",
                    display:
                      this.state.customer_voucher.length === 0 ? "none" : "flex"
                  }}
                  onPress={() => {
                    LayoutAnimation.configureNext(
                      LayoutAnimation.Presets.easeInEaseOut
                ...
                <Button
                  style={{
                    ...
                    alignItems: "center",
                    display:
                      this.state.customer_voucher_history.length === 0
                        ? "none"
                        : "flex"
                  }}
                  onPress={() => {
                    ...
                    //this.editCustomer();
                    this.setState({ showVoucherHistory: true });
                  }}
                >
                  <Text
                    style={[
                  ...
                  >
                    {_voucher_history[this.state.languageIndex]}
                  </Text>
                </Button>
              </View>
              ...
              >
                <Button
                  style={{
                    ...
                    alignItems: "center",
                    display:
                      this.state.customer_voucher_redeem.length === 0
                        ? "none"
                        : "flex"
                  }}
                  onPress={() => {
                    ...
                    //this.editCustomer();
                    this.setState({ showVoucherRedeem: true });
                  }}
                >
                  <Text
                    style={[
                  ...
                  >
                    {_voucher_redeem[this.state.languageIndex]}
                  </Text>
                </Button>
                <Button
                  style={{
  ...
  renderModalVoucherHistory() {
    return (
      <Modal
        animationType="slide"
        transparent={true}
        //visible={this.state.showForm}
        visible={this.state.showVoucherHistory}
        onRequestClose={() => {
          this.setState({ showVoucherHistory: false });
        }}
      >
        <View style={{ flex: 1, backgroundColor: WHITE, paddingBottom: 15 }}>
          <MobileHeader
            colorIndex={this.state.colorIndex}
            title={_voucher_history[this.state.languageIndex]}
            notif={false}
            hideLogin={true}
            loginInformation={this.state.userInfo}
            menu={false}
          ...
          <View style={{ padding: 15, marginBottom: 30, paddingBottom: 0 }}>
            {this.state.tablet ? (
              <FlatList
                //ListHeaderComponent={this.renderSearch()}
                showsVerticalScrollIndicator={false}
                data={this.state.customer_voucher_history}
                columnWrapperStyle={{
                  justifyContent: "space-between",
                  marginBottom: 5,
                  marginLeft: 15,
                ...
                numColumns={2}
                renderItem={({ item, index }) => {
                  return this.renderVoucherHistory(item, index);
                }}
                //ListFooterComponent={this._renderFooter}
                keyExtractor={(item, index) => {
  ...
  renderModalVoucherRedeem() {
    return (
      <Modal
        animationType="slide"
        transparent={true}
        //visible={this.state.showForm}
        visible={this.state.showVoucherRedeem}
        onRequestClose={() => {
          this.setState({ showVoucherRedeem: false });
        }}
      >
        <View style={{ flex: 1, backgroundColor: WHITE, paddingBottom: 15 }}>
          <MobileHeader
            colorIndex={this.state.colorIndex}
            title={_voucher_redeem[this.state.languageIndex]}
            notif={false}
            hideLogin={true}
            loginInformation={this.state.userInfo}
            menu={false}
          ...
          <View style={{ padding: 15, marginBottom: 30, paddingBottom: 0 }}>
            {this.state.tablet ? (
              <FlatList
                //ListHeaderComponent={this.renderSearch()}
                showsVerticalScrollIndicator={false}
                data={this.state.customer_voucher_redeem}
                columnWrapperStyle={{
                  justifyContent: "space-between",
                  marginBottom: 5,
                  marginLeft: 15,
                ...
                numColumns={2}
                renderItem={({ item, index }) => {
                  return this.renderVoucherRedeemList(item, index);
                }}
                //ListFooterComponent={this._renderFooter}
                keyExtractor={(item, index) => {
                  return "History_Point" + index.toString();
  ...
  renderModalVoucher() {
    return (
      <Modal
        ...
        transparent={true}
        //visible={this.state.showForm}
        visible={this.state.showVoucher}
        onRequestClose={() => {
      ...
      >
        <View style={{ flex: 1, backgroundColor: WHITE, paddingBottom: 15 }}>
          <MobileHeader
            colorIndex={this.state.colorIndex}
            title={_voucher[this.state.languageIndex]}
            notif={false}
            hideLogin={true}
            loginInformation={this.state.userInfo}
            menu={false}
          ...
          <View style={{ padding: 15, marginBottom: 30, paddingBottom: 0 }}>
            {this.state.tablet ? (
              <FlatList
                ...
                numColumns={2}
                renderItem={({ item, index }) => {
                  return this.renderVoucherList(item, index);
                }}
                //ListFooterComponent={this._renderFooter}
                keyExtractor={(item, index) => {
                  return "History_Point" + index.toString();
  ...
  renderVoucherList(data, index) {
    ...
    //if (data.points) {
    total = data.Customer_Voucher_List.discount_amount;

    let type = data.Customer_Voucher_List.discount_type;

    let discount_type = type === "amount" ? "" : "%";

    let description = data.Customer_Voucher_List.description;
    //}
  ...
  renderVoucherRedeemList(data, index) {
    //console.log("Render History ==> ", data);
    let time = "";
    ...
    let purchase_type = data.purchase_type;
    if (purchase_type === "currency") {
      purchase_type = "Purchased";
    }

    if (purchase_type === "point") {
      purchase_type = "Points";
    }

    if (purchase_type === "claim") {
      purchase_type = "Claim";
    }

    if (purchase_type === "system") {
      purchase_type = "System";
    }

    if (purchase_type === "automatic") {
      purchase_type = "Automatic";
    }
        ...
        >
          ...
          >
            ...
            <View style={{ width: "33%", alignItems: "center" }}>
              ...
              >
                {_claim_method[this.state.languageIndex]}
              </Text>
              <Text
                style={[
                  MainStyle.robotoNormal,
  ...
  renderVoucherHistory(data, index) {
    //console.log("Render History ==> ", data);
    let time = "";
